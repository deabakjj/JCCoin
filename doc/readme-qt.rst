Rajmudra-qt: Qt4 GUI for Rajmudra
===============================
...
Alternatively, install Qt Creator and open the `Rajmudra-qt.pro` file.

An executable named `Rajmudra-qt` will be built.
...
A warning for people using the *static binary* version of Rajmudra on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Rajmudra is linked against libdb5.3.
...
installed causes Rajmudra-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
